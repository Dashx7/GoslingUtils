        ...
        if len(agent.stack) < 1:
            ...
            if agent.kickoff_flag:
                ...
                if len(shots["goal"]) > 0:
                    ...
                    def score_shot(agent, shot):
                        me = agent.me
                        ball = shot.ball
                        target = shot.target

                        # Direction from ball to target (enemy goal)
                        goal_direction = (target - ball.location).normalize()
                        
                        # Shot vector: where the ball is going to be hit
                        shot_vector = (target - me.location).normalize()

                        # Alignment: how well the shot matches goal direction
                        alignment = shot_vector.dot(goal_direction)

                        # Speed score: how fast the ball is already moving toward the enemy goal
                        ball_velocity = ball.velocity.dot(goal_direction)
                        speed_score = clamp01(ball_velocity / 3000)

                        # Time until intercept
                        time_score = clamp01(1 - shot.time / 6)

                        # Angle sharpness penalty (prefer straight-on shots)
                        angle_penalty = clamp01(1 - abs(alignment))  # lower is better

                        # Optional: check for open net (no enemy between ball and goal)
                        open_net_bonus = 0
                        for enemy in agent.foes:
                            if is_between(enemy.location, ball.location, target):
                                open_net_bonus = -0.5
                                break
                        else:
                            open_net_bonus = 0.5  # reward open nets
                    ...
                    if best_shot is not None:
                        agent.line(agent.me.location, best_shot.target, colors.Green)
                        agent.renderer.draw_string_3d(best_shot.target, 2, 2, "AIM", colors.White)
...
def is_between(p, a, b, buffer=200):
    # Check if point p is between a and b within a buffer (like a loose segment check)
    ap = p - a
    ab = b - a
    ab_norm = ab.normalize()
    proj_len = ap.dot(ab_norm)
    return 0 < proj_len < ab.length() and (ap - ab_norm * proj_len).length() < buffer
